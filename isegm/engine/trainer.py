...
class ISTrainer(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            # loss = self.add_loss('mid_loss', loss, losses_logging, validation,
            #                      lambda: (output['mid_pred'], batch_data['instances']))
            # prior_gt = self.get_prior_gt(orig_gt_mask,image)
            # B = image.shape[0]
            # prior_f,prior_b = torch.ones(B,1,768),torch.zeros(B,1,768)
            # prior_gt = torch.cat((prior_f,prior_b),dim=1).cuda()
            loss = self.add_sim_loss('prior_loss', loss, losses_logging, validation, output['instances_aux'])
    ...
    #                 # similar_weights = torch.exp(-dist_min)
    #                 if len(y) < image.shape[0]:
    ...
    def add_sim_loss(self, loss_name, total_loss, losses_logging, validation, loss_inputs):
        loss_cfg = self.loss_cfg if not validation else self.val_loss_cfg
        loss_weight = loss_cfg.get(loss_name + '_weight', 0.0)
        if loss_weight > 0.0:
            loss_criterion = loss_cfg.get(loss_name)
            loss = loss_criterion(loss_inputs)
            loss = torch.mean(loss)
            losses_logging[loss_name] = loss
            loss = loss_weight * loss
            total_loss = total_loss + loss
    ...
    def get_prior_gt(self,orig_gt_mask,image):
        g_f,g_b = orig_gt_mask,1-orig_gt_mask
        g = torch.cat((g_f,g_b),dim=1)
        B,C,H,W = orig_gt_mask.shape
        gf = g.view(B,2,H//16,16,W//16,16)
        gf = torch.mean(gf,3)
        gf = torch.mean(gf,-1)
        gf = gf.flatten(2).transpose(2,1)
        x = image.view(B,3,H//16,16,W//16,16)
        x = x.permute(0,2,4,1,3,5).flatten(3)
        x = x.permute(0,3,1,2).flatten(2).transpose(2,1)
        x_f, x_b = x*gf[:,:,0].unsqueeze(-1), x*gf[:,:,1].unsqueeze(-1)
        # x_f, x_b = x_f / (x_f+x_b+0.00001),x_b / (x_f+x_b+0.00001)
        x_f, x_b = x_f.transpose(2,1).mean(-1),x_b.transpose(2,1).mean(-1)
        x = torch.cat((x_f.unsqueeze(1),x_b.unsqueeze(1)),dim=1)
        return x
