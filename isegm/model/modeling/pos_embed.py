...
def interpolate_pos_embed(model, checkpoint_model):
    if 'pos_embed' in checkpoint_model:
        ...
        if num_extra_tokens - 2 > 1:
            cls_token=checkpoint_model["cls_token"]
            extra_cls_tokens = nn.Parameter(torch.zeros(1, num_extra_tokens-3, embedding_size))
            new_cls_token = torch.cat((extra_cls_tokens,cls_token),dim=1)
            checkpoint_model["cls_token"] = new_cls_token

        if num_extra_tokens > 1:
            num_extra_token = 1
        else:
            num_extra_token = num_extra_tokens
        # height (== width) for the checkpoint position embedding
        orig_size = int((pos_embed_checkpoint.shape[-2] - num_extra_token) ** 0.5)
        # height (== width) for the new position embedding
        new_size = int(num_patches ** 0.5)
        extra_tokens = nn.Parameter(torch.zeros(1, num_extra_tokens, embedding_size))
        pos_tokens = pos_embed_checkpoint[:, num_extra_token:]
        # class_token and dist_token are kept unchanged
        if orig_size != new_size:
            print("Position interpolate from %dx%d to %dx%d" % (orig_size, orig_size, new_size, new_size))
            # extra_tokens = pos_embed_checkpoint[:, :num_extra_token]
