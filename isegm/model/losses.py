...
class Sim_Loss(nn.Module):
    ...
    def forward(self,x):
        # x = self.sigmoid(x)
        x_f,x_b = x[:,0],x[:,1]
        u1,u2 = torch.mean(x_f,-1), torch.mean(x_b,-1)
        # jb = torch.mean(torch.abs(u1-u2))
        jb = torch.mean(torch.exp(-(((u1-u2)**2)-10)/100))
        s1 = torch.mean((x_f-u1.unsqueeze(-1).repeat([1,x_f.shape[-1]]))**2,-1)
        s1 = torch.mean(s1)
        s2 = torch.mean((x_b-u2.unsqueeze(-1).repeat([1,x_b.shape[-1]]))**2,-1)
        s2 = torch.mean(s2)
        jw = s1 + s2
        # loss = torch.exp(-((x_f-x_b)**2)/0.1)
