...
def parse_args():
    ...
    #                     help='')
    parser.add_argument('--mode',type=str, default='NoBRS',help='')

    # group_checkpoints = parser.add_mutually_exclusive_group(required=True)
    parser.add_argument('--checkpoint', type=str, default='prior_tokenize_cross_attention_cl_large.pth',
    ...
    # parser.add_argument('--checkpoint', type=str, default='004.pth',
    #                                help='The path to the checkpoint. '
    #                                     'This can be a relative path (relative to cfg.INTERACTIVE_MODELS_PATH) '
    #                                     'or an absolute path. The file extension can be omitted.')
    ...
    parser.add_argument('--datasets', type=str, default='ssTEM,BraTS,OAIZIB',
    ...
    # parser.add_argument('--datasets', type=str, default='GrabCut,Berkeley,DAVIS',
    #                     help='List of datasets on which the model should be tested. '
    #                          'Datasets are separated by a comma. Possible choices: '
    #                          'GrabCut, Berkeley, DAVIS, SBD, PascalVOC')
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    ...
    def callback(image, gt_mask, pred_probs, sample_id, click_indx, clicks_list,iou):
        sample_path = save_path / f'{sample_id}_{click_indx}.jpg'
        prob_map = draw_probmap(pred_probs)
        click_text = f'{click_indx+1} click' if click_indx==0 else f'{click_indx+1} clicks'
        iou_text = 'IoU={:.2f}%'.format(iou*100)
        image_with_mask = draw_with_blend_and_clicks(image, pred_probs > prob_thresh, clicks_list=clicks_list)
        h,w,_ = image_with_mask.shape
        HS = h-20
        WS = 20
        cv2.putText(image_with_mask,click_text,(WS,HS),cv2.FONT_HERSHEY_SIMPLEX,1,(225,225,225),2,cv2.LINE_AA)
        cv2.putText(prob_map,iou_text,(WS,HS),cv2.FONT_HERSHEY_SIMPLEX,1,(225,225,225),2,cv2.LINE_AA)
